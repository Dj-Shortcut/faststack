...
def test_startup_only_one_scan(controller):
    ...
    with patch('faststack.app.find_images_with_variants', return_value=([], {})) as mock_scan:
        controller.load()
...
def test_loupe_filter_handles_dirty_flag(controller):
    ...
    print(f"DEBUG: before apply_filter: grid_active={controller._is_grid_view_active}, dirty={controller._grid_model_dirty}")
    controller.apply_filter("test", [])
    print(f"DEBUG: after apply_filter: grid_active={controller._is_grid_view_active}, dirty={controller._grid_model_dirty}")
    
    # Grid should NOT have refreshed
    assert not controller._thumbnail_model.refresh_from_controller.called, "refresh_from_controller should NOT have been called"
    assert controller._grid_refreshes == 0, f"grid_refreshes should be 0, got {controller._grid_refreshes}"
    # Dirty flag should be set
    assert controller._grid_model_dirty is True, "grid_model_dirty should be True after filtering in loupe mode"
