...
def test_handle_delete_failures_recycle_codes():
    ...
    controller._handle_delete_failures = AppController._handle_delete_failures.__get__(controller, AppController)
    ...
    with patch("faststack.app.confirm_permanent_delete", return_value=True) as mock_confirm:
        ...
        assert 123 in controller._pending_delete_jobs, "Job should be stored in pending map"
        assert controller._delete_executor.submit.called, "Should have submitted perm delete worker"
        assert fut.add_done_callback.called, "Should have registered callback on the future"
    ...
    with patch("faststack.app.confirm_permanent_delete", return_value=True) as mock_confirm:
        ...
        assert controller._rollback_ui_items.called, "Should have rolled back UI for non-recycle error"
        assert 123 not in controller._pending_delete_jobs, "Job should NOT be kept pending for non-recycle error"
        assert not controller._delete_executor.submit.called, "Should NOT submit perm delete for non-recycle error"
