...
class TestEditorErrorHandling(unittest.TestCase):
    """Test that ImageEditor raises exceptions when loading fails completely."""

    def test_load_image_raises_exception_on_failure(self):
        """Ensure load_image raises an exception when file opening fails completely."""
        ...
        with patch('PIL.Image.open', side_effect=OSError("Mocked file error")):
             ...
             with patch.dict(sys.modules, {'cv2': MagicMock()}):
                 ...
                 self.assertIn("Mocked file error", str(cm.exception))
                 
    def test_save_image_raises_exception_on_failure(self):
        """Ensure save_image raises an exception when saving fails."""
        from faststack.imaging.editor import ImageEditor
