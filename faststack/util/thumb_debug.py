...
def gauge(name: str, value: Any):
    ...
    with _stats_lock:
        if name in _interval_stats:
            ...
            if name == "qdepth":
                _interval_stats["qdepth_max"] = max(_interval_stats["qdepth_max"], value)
...
class ThumbTimer:
    ...
    def log_timing(self, **kwargs):
        ...
        if self.prio_effective is not None and self.prio_effective != self.prio_submitted:
...
def check_periodic_summary():
    ...
    with _stats_lock:
        ...
        for k in [
            "req_total", "req_cache_hit", "req_cache_miss",
            "decode_submitted", "decode_coalesced", "decode_started",
            "decode_done_ok", "decode_done_fail", "decode_cancelled",
            "total_ms", "max_ms", "qdepth_max"
        ]:
    ...
    activity = (
        stats["req_total"] + stats["decode_submitted"] + 
        stats["decode_done_ok"] + stats["decode_cancelled"] + 
        stats["inflight"]
    )
    if activity == 0:
        return

    avg_ms = stats["total_ms"] / stats["decode_done_ok"] if stats["decode_done_ok"] > 0 else 0
